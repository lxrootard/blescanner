...
function pauseGraph() {
    if (renderer) {
	const text = document.getElementById("animate_text");
	const icon = document.getElementById("animate_icon");
...
function loadGraph() {
    const MAX_RSSI = -100;
    const NO_RSSI = MAX_RSSI * 2;
    const MAX_DIST = 50;
    const scale = 0.7;
    const scaleX = ((window.innerWidth - 200)/2) * scale;
    const scaleY = ((window.innerHeight - 200)/2) * scale;
    const selectedLink = '#0066cc'; //'blue';
    const aliveLink = 'grey';
    const selectedText = '#0066cc'; // 'blue';
    const aliveText = 'grey';
    const deadText = 'red';
    const nodeSize = 50; // taille des noeuds
    const idealLength = 400; // taille des links
	...
	graph.forEachLinkedNode(nodeId, function (node2, link) {
		...
		if (linkUI) {
			...
			svgText = Viva.Graph.svg('text').text(node2.id).attr('fill', node2.data.txtColor);
			let l1 = document.getElementById(nodeId);
			let n1 = graph.getNode(nodeId);
			l1.text(isOn? nodeId : n1.data.label).attr('fill', isOn? selectedText : aliveText);
			let l2 = document.getElementById('label_'+ linkUI.attr('id'));
			if (l2 != null) {
				l2.text(linkUI.link.data.label);
				l2.attr('fill', isOn? selectedText : 'none').attr('x', linkUI.attr('x')).attr('y', linkUI.attr('y'));
				//console.log('l2= %o',l2);
			}
	...
	svgText = Viva.Graph.svg('text').text(node.data.label).attr('id',  node.data.id).attr('fill', node.data.txtColor),
	img = Viva.Graph.svg('image').attr('width', nodeSize).attr('height', nodeSize).link(node.data.url);
	...
	graphics.link(function(link) {
		const label = Viva.Graph.svg('text').attr('id','label_'+ link.data.id); // .text(link.data.label);
		graphics.getSvgRoot().childNodes[0].append(label);
		return Viva.Graph.svg('path')
	   		.attr('stroke', aliveLink)
	   		.attr('id', link.data.id);
			}).placeLink(function(linkUI, fromPos, toPos) {
		    		const toNodeSize = nodeSize,fromNodeSize = nodeSize;
...
//	alert ('nodes:' + JSON.stringify(nodes));
        ...
        // Populate the graph with some nodes and links:
			for (a in antennas) {
				const dead = (antennas[a]['online'] == false);
				const color = (dead)? deadText : aliveText;
				const name = antennas[a]['name'];
				const picture = antennas[a]['picture'];
				graph.addNode(a, { id: a, label: name, url: picture, isPinned: dead, txtColor: color });
			}
			for (n in nodes) {
				const dead = (nodes[n]['rssi'] == NO_RSSI);
				const color = (dead)? deadText : aliveText;
				const name = nodes[n]['name'];
				const picture =  nodes[n]['picture'];
				graph.addNode(n,{ id: n, label: name, url: picture, isPinned: dead, txtColor: color });

				if (dead) { // dead
					const x = Math.floor(Math.random() * randomNumber(-1,1) * scaleX);
					const y = Math.floor(Math.random() * randomNumber(-1,1) * scaleY);
				...
				for (a in antennas) {
					if (mode == 'Distance') {
						...
						if ((!isNaN(dist)) && (dist != -1))
							graph.addLink(n, a, { id: n + '-' + a, connectionStrength: dist/MAX_DIST, label: dist + 'm'});
					} else {
